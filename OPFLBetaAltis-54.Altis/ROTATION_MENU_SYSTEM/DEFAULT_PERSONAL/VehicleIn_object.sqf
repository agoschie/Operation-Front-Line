...
private ["_return", "_class", "_text", "_modelPosition", "_icon", "_menuItems", "_obj", "_mainColor", "_mainScript", "_distance", "_condition", "_conditionHide"];

_class = "RTMS_PERSONAL"; //put an object here, such as a soldier or vehicle
...
_icon = "A3\ui_f\data\gui\Rsc\RscDisplayArsenal\face_ca.paa"; //This is the icon texture path. Replace this with a similar path to another icon, or make a custom one
...
_distance = 10;  //distance before you can see and select the icon AND sub menus! 
...
_conditionHide = [[], compileFinal "true"];
...
private ["_menuA", "_menuB"];

_menuB = [
			["   Lock", RTMS_DEFAULT_SUB_COLOR, RTMS_DEFAULT_SUB_SIZE, RTMS_DEFAULT_SUB_TEXT_SIZE, "PuristaMedium", {(_this select 0) lock 2;}, [], _distance, [[], {(locked vehicle player) == 0}], RTMS_DEFAULT_SUB_ICON],
			["   Unlock", RTMS_DEFAULT_SUB_COLOR, RTMS_DEFAULT_SUB_SIZE, RTMS_DEFAULT_SUB_TEXT_SIZE, "PuristaMedium", {(_this select 0) lock 0;}, [], _distance, [[], {(locked vehicle player) != 0}], RTMS_DEFAULT_SUB_ICON],
			["   Disembark", RTMS_DEFAULT_SUB_COLOR, RTMS_DEFAULT_SUB_SIZE, RTMS_DEFAULT_SUB_TEXT_SIZE, "PuristaMedium", {if (2 == locked (_this select 0)) exitWith {hint "ACCESS: Locked";}; player action ["getOut", _this select 0];}, [], _distance, [], RTMS_DEFAULT_SUB_ICON]
		];
		
_menuCA = [
			["ON", RTMS_DEFAULT_SUB_COLOR, RTMS_DEFAULT_SUB_SIZE, RTMS_DEFAULT_SUB_TEXT_SIZE, "PuristaMedium", RTMS_ScrollOverrideOn, [], _distance, [[], {!RTMS_MOUSE_SCROLL_OVERRIDE}], RTMS_DEFAULT_SUB_ICON],
			["OFF", RTMS_DEFAULT_SUB_COLOR, RTMS_DEFAULT_SUB_SIZE, RTMS_DEFAULT_SUB_TEXT_SIZE, "PuristaMedium", RTMS_ScrollOverrideOff, [], _distance, [[], {RTMS_MOUSE_SCROLL_OVERRIDE}], RTMS_DEFAULT_SUB_ICON]
		];

_menuC = [
			["   Scroll Override", RTMS_DEFAULT_SUB_COLOR, RTMS_DEFAULT_SUB_SIZE, RTMS_DEFAULT_SUB_TEXT_SIZE, "PuristaMedium", RTMS_PushSubMenu, [_menuCA], _distance, [], RTMS_DEFAULT_SUB_ICON]
		];


_menuItems = 
	[
		["  EJECT!", RTMS_DEFAULT_SUB_COLOR, RTMS_DEFAULT_SUB_SIZE, RTMS_DEFAULT_SUB_TEXT_SIZE, "PuristaMedium", {if (2 == locked (_this select 0)) exitWith {hint "ACCESS: Locked";}; player action ["Eject", (_this select 0)];}, [], _distance, [], RTMS_DEFAULT_SUB_ICON],
		["  Access", RTMS_DEFAULT_SUB_COLOR, RTMS_DEFAULT_SUB_SIZE, RTMS_DEFAULT_SUB_TEXT_SIZE, "PuristaMedium", RTMS_PushSubMenu, [_menuB], _distance, [[], {(driver vehicle player) == player}], RTMS_DEFAULT_SUB_ICON],
		["  RTMS Options", RTMS_DEFAULT_SUB_COLOR, RTMS_DEFAULT_SUB_SIZE, RTMS_DEFAULT_SUB_TEXT_SIZE, "PuristaMedium", RTMS_PushSubMenu, [_menuC], _distance, [], RTMS_DEFAULT_SUB_ICON]
	];
	
	
	
//these lines return all the settings in this menu	
_return = [_class, _text, _modelPosition, _icon, _menuItems, _mainColor, _mainScript, _distance, _condition, _conditionHide];
_return;