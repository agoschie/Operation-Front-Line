...
Dear user,
	...
	1.  - Only use the main script if your menu will be the only one attached to objects of the specified class AND you have no sub menus. (just a lone icon)
	...
	3.  - Priority is given to direct object menus first.
	    - A direct object menu uses an object instead of a "_class" string like in this template. They get directly attached to that single object, and never get deallocated. (see Template_Object.sqf in the other folder)
...
private ["_return", "_class", "_text", "_modelPosition", "_icon", "_menuItems", "_obj", "_mainColor", "_mainScript", "_distance", "_condition", "_conditionHideIcon"];

_class = "Man"; //put a "class" or a "kind" in these quotes (example, "Air", "Car", "Tank", "Plane", "Helicopter", "Man", "Offroad_01")

_text = "EXPAND"; //text you want to display on the icon if this menu has priority

_modelPosition = [0,0,1.4]; //where, in model space, you want to display the menu icon

_icon = "\A3\ui_f\data\IGUI\Cfg\Cursors\explosive_ca.paa"; //This is the icon texture path. Replace this with a similar path to another icon, or make a custom one
...
_distance = 3;  //distance before you can see and select the icon AND sub menus! 
...
//first sub menu in "_menuItems" is the automatic default actions (like driver, gunner, commander, ect), notice its always the first sub menu item (if you want the default actions) and has an empty "" text string in the beginning
//second sub menu in "_menuItems" is a custom action with the display text "Inventory". It is already made for you to see and opens the objects inventory.
...
_return = [_class, _text, _modelPosition, _icon, _menuItems, _mainColor, _mainScript, _distance, _condition, _conditionHideIcon];
_return;