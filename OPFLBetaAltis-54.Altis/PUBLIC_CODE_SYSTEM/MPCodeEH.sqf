//"PCS_PUBLIC_EX" addPublicVariableEventHandler
(format ["PCS_KA_%1", _this]) addPublicVariableEventHandler
{
	private "_publicName";
	...
	if (!isNil "_machine") exitWith
	{
		...
		if (!isDedicated) then
		{
			...
			if (_me) then
			{
				...
				{
					[[_publicValue, _type, false], "PCS_SendData"] call PCS_CallFnc;
				};
				_null = [_scriptArguments, _scriptEx] spawn PCS_CallFnc;
			}
			else
			{
				//diag_log "MPCodeEH: Unsupported address";
				[[_publicValue, _type, false], "PCS_SendData"] call PCS_CallFnc;
			};
		}
		else
