...
//
//    • KRON_getArg     - Searches a mixed array (_this) for a matching string beginning with (_t), and returns the part after a separator (":")
...
KRON_StrMid = {
	...
	_arr=[_in] call KRON_StrToArray;
	_len=count(_arr)-1;
	if ((count _this)>2) then {_len=(_this select 2)-1};
	_out="";
	if ((_pos+_len)>=(count _arr)) then {_len=(count _arr)-_pos-1};
	if (_len>0) then {
		for "_i" from _pos to (_pos+_len) do {
			_out=_out + (_arr select _i);
		};
	};
	_out
		...
		for "_i" from 0 to (count _in)-1 do {
			...
			if (typeName _as=="STRING") then {
				...
				if (_p+1==(count _aa)-_fl) then {
					if (([_as,_p] call KRON_StrMid)==_flg) then {
						_org = _in select _i;
