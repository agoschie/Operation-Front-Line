...
_menuItems = 
	[
		["  DEPLOY RP", RTMS_DEFAULT_SUB_COLOR, RTMS_DEFAULT_SUB_SIZE, RTMS_DEFAULT_SUB_TEXT_SIZE, "PuristaMedium", {if (!isNil "OPFL_Client_AttemptDeployRP") then {call OPFL_Client_AttemptDeployRP;};}, [], _distance, [[], {alive player && OPFL_RP_READY}], RTMS_DEFAULT_SUB_ICON]
	];
	
	
	
//these lines return all the settings in this menu	
_return = [_class, _text, _modelPosition, _icon, _menuItems, _mainColor, _mainScript, _distance, _condition, _conditionHide, _isInternal];
_return;