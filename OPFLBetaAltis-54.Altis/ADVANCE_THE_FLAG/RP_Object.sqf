...
private ["_return", "_text", "_modelPosition", "_icon", "_menuItems", "_obj", "_mainColor", "_mainScript", "_distance", "_condition", "_conditionHideIcon", "_isInternal"];

_obj = "FlagSmall_F"; //put an object here, such as a soldier or vehicle
...
_mainScript = [[], OPFL_Client_AttemptVote]; //takes form [[ARGS], CODE], otherwise empty array []
...
_condCode =
{
	private ["_rp", "_person", "_return"];
	_return = true;
	_rp = _this select 0;
	if (isNil {_rp getVariable "RP_PLAYER"}) exitWith {_return;};
	
	_person = _rp getVariable "RP_PLAYER";
	if (isNull _person) exitWith {_return};
	_return = !((player != _person) && (alive _person) && (_rp getVariable "RP_CAN_VOTE"));
	_return;
};

_conditionHideIcon = [[], _condCode];
_isInternal = false;
...
_return = [_obj, _text, _modelPosition, _icon, _menuItems, _mainColor, _mainScript, _distance, _condition, _conditionHideIcon, _isInternal];
_return;