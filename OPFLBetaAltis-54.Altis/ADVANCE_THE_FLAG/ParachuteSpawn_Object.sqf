...
_class = objNull; //put an object here, such as a soldier or vehicle
...
_icon = "a3\ui_f\data\Map\VehicleIcons\pictureParachute_ca.paa"; //This is the icon texture path. Replace this with a similar path to another icon, or make a custom one

_mainColor = RTMS_DEFAULT_COLOR; //color of the icon if this menu has priority, currently set at default

_mainScript = [[], OPFL_Client_ParachuteSpawn]; //takes form [[ARGS], CODE], otherwise empty array []
				  //[ARGS] automatically have the attached object as the first argument (_this select 0)

_distance = 6;  //distance before you can see and select the icon AND sub menus! 
