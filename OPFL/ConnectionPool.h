...
namespace active911 {
	...
	struct ConnectionImpossible : std::exception {
				...
				for (std::set<std::shared_ptr<Connection> >::iterator it = this->borrowed.begin(); it != this->borrowed.end(); ++it) {
					...
					if ((*it).unique()) {
						...
						try {
							...
							//	*** NEED TO CHANGE THIS TO ConnectionImpossible ***
							//	*** IF THE DATABASE IS DOWN, THIS SHOULD BE A DIFFERENT ERROR ***
